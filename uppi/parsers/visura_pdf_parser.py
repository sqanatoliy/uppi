...
class VisuraParser:
    ...
    def _extract_name_cf(self, doc) -> Dict[str, Any]:
        ...
        for _, _, _, _, text, *_ in blocks:
            for line in text.splitlines():
                match = self.NAME_CF.match(line.strip())
                if match:
                    locatore_surname, locatore_name, cf = match.groups()
    ...
    def _extract_comune_for_page(self, page) -> tuple[str | None, str | None]:
        ...
        for line in txt.splitlines():
            match = self.COMUNE_TABLE.search(line.strip())
            if match:
        ...
        for i in range(data_start_row, len(df)):
            ...
            for col_index, col_name in enumerate(header):
                ...
                if "indirizzo" in col_name:
                    row_dict.update(self._parse_address(raw_value))
                    continue
    ...
    def _parse_address(self, text: str) -> Dict[str, Any]:
        ...
        return {
            "via_type": parsed.via_type,
            "via_name": parsed.via_name,
            "via_num": parsed.civico,
            "scala": parsed.scala,
            "interno": parsed.interno,
            "piano": parsed.piano,
            "indirizzo_raw": parsed.indirizzo_raw,
        }
